...
try {
    ...
    if (!$checkTable) {
        echo json_encode(['success' => true, 'count' => 0, 'message' => 'Таблица favorites не найдена']);
        exit;
    }
    
    // Получаем количество избранного
    $countQuery = "SELECT COUNT(*) as count FROM favorites WHERE user_id = :user_id";
