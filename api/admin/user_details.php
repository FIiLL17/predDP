...
// ========== ДОБАВЛЯЕМ ОТЗЫВЫ ==========
$reviews = $pdo->prepare("
    SELECT id, rating, comment, is_verified, status, created_at
    FROM reviews
    WHERE user_id = ?
...
<head>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; font-family:'Segoe UI',Arial,sans-serif; }
        ...
        .badge.approved { background:rgba(0,204,102,0.2); color:#0c6; border:1px solid #0c6; }
        .badge.rejected { background:rgba(255,51,102,0.2); color:#f36; border:1px solid #f36; }
        ...
        .star-rating { color: gold; letter-spacing: 2px; }
    </style>
</head>
<body>
    <div class="container">
        ...
        <div class="tabs">
            ...
            <!-- Новая вкладка -->
            <div class="tab" data-tab="reviews">Отзывы (<?= count($reviews) ?>)</div>
        </div>
        ...
        <!-- Новая вкладка: Отзывы и обратная связь -->
        <div id="reviews" class="tab-content">
            <h3 style="color: var(--admin-blue); margin-bottom: 20px;">Отзывы и обратная связь пользователя</h3>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="badge <?= $review['status'] ?>">
                                    <?php
                                        $statusMap = [
                                        ...
                                        echo $statusMap[$review['status']] ?? $review['status'];
                                    ?>
                                </span>
                            </td>
                            <td><?= $review['is_verified'] ? '<span class="badge active">Да</span>' : '<span class="badge inactive">Нет</span>' ?></td>
            ...
            <?php else: ?>
                <p style="text-align:center; padding:40px; color:#aaa;">Пользователь ещё не оставлял отзывов</p>
            <?php endif; ?>
        </div>
